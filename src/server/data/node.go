...
func (node *Node) Resolve(sc SearchContext, key interface{}) (interface{}, error) {
	...
	if !node.OnlyLink {
		...
		if keyStr, err := node.keyAsString(obj, objectMeta); err != nil {
			...
			if node.IsOfGenericType() {
				return map[string]string{types.GenericInnerLinkObjectKey: objectMeta.Name, types.GenericPkKey: keyStr}, nil
			} else {
				return keyStr, nil
			}
		}
...
func (node *Node) ResolveRegularPlural(sc SearchContext, key interface{}) ([]interface{}, error) {
	...
	if records, err := sc.dm.GetAll(node.Meta, fields, map[string]interface{}{node.KeyField.Name: key}); err != nil {
		return nil, err
	} else {
