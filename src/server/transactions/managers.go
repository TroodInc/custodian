...
type MetaDescriptionTransactionManager interface {
	BeginTransaction() (MetaDescriptionTransaction, error)
...
func (g *GlobalTransactionManager) BeginTransaction() (*GlobalTransaction, error) {
	if g.transaction == nil {
		dbTransaction, err := g.DbTransactionManager.BeginTransaction()
		if err != nil {
			return nil, err
		}
		metaDescriptionTransaction, err := g.MetaDescriptionTransactionManager.BeginTransaction()
		if err != nil {
			return nil, err
		}
